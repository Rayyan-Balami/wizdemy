...
CREATE TABLE `follow_relation` (
  `follow_relationship_id` int(11) NOT NULL,
...
,`requests_count` bigint(21)
,`materials_count` bigint(21)
,`project_count` bigint(21)
,`responded_requests_count` bigint(21)
);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `user_profile_view`  AS SELECT `u`.`user_id` AS `user_id`, `u`.`status` AS `status`, `u`.`private` AS `private`, `u`.`allow_email` AS `allow_email`, `u`.`allow_phone` AS `allow_phone`, `u`.`username` AS `username`, `u`.`full_name` AS `full_name`, `u`.`email` AS `email`, `u`.`phone_number` AS `phone_number`, `u`.`education_level` AS `education_level`, `u`.`enrolled_course` AS `enrolled_course`, `u`.`bio` AS `bio`, `u`.`created_at` AS `created_at`, `u`.`user_type` AS `user_type`, count(distinct `f1`.`follower_id`) AS `followers_count`, count(distinct `f2`.`following_id`) AS `following_count`, count(distinct `r`.`request_id`) AS `requests_count`, count(distinct `m`.`material_id`) AS `materials_count`, count(distinct `p`.`project_id`) AS `project_count`, count(distinct case when `s`.`request_id` is not null then `s`.`user_id` end) AS `responded_requests_count` FROM (((((((`users` `u` left join `follow_relation` `fr` on(`u`.`user_id` = `fr`.`following_id`)) left join `follow_relation` `f1` on(`f1`.`following_id` = `u`.`user_id`)) left join `follow_relation` `f2` on(`f2`.`follower_id` = `u`.`user_id`)) left join `study_material_requests` `r` on(`r`.`user_id` = `u`.`user_id`)) left join `study_materials` `m` on(`m`.`user_id` = `u`.`user_id`)) left join `github_projects` `p` on(`p`.`user_id` = `u`.`user_id`)) left join `study_materials` `s` on(`s`.`user_id` = `u`.`user_id` and `s`.`request_id` is not null)) GROUP BY `u`.`user_id`, `u`.`status`, `u`.`private`, `u`.`allow_email`, `u`.`allow_phone`, `u`.`username`, `u`.`full_name`, `u`.`email`, `u`.`phone_number`, `u`.`education_level`, `u`.`enrolled_course`, `u`.`bio`, `u`.`created_at`, `u`.`user_type` ;
...
--
ALTER TABLE `follow_relation`
  MODIFY `follow_relationship_id` int(11) NOT NULL AUTO_INCREMENT;
...
--
ALTER TABLE `admin_action_log`
  ADD CONSTRAINT `admin_action_log_ibfk_1` FOREIGN KEY (`admin_id`) REFERENCES `admins` (`admin_id`) ON DELETE CASCADE;
...
--
ALTER TABLE `likes`
  ADD CONSTRAINT `likes_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`) ON DELETE NO ACTION,
...
ALTER TABLE `reports`
  ...
  ADD CONSTRAINT `reports_ibfk_2` FOREIGN KEY (`material_id`) REFERENCES `study_materials` (`material_id`) ON DELETE CASCADE;
